...
(() => {
  ...
  function post(payload){if(window.opener)window.opener.postMessage(payload,target)}
  ...
  document.getElementById("pin-form").addEventListener("submit",event=>{event.preventDefault();const pin=document.getElementById("pin").value;if(pin==="608"){post({type:"functionhx:spark-vault-decoy"});setStatus("已打开私密空间。","success");setTimeout(()=>window.close(),260);return}setStatus("访问码不正确。","error")});
  ...
  form.addEventListener("submit",async event=>{event.preventDefault();submit.disabled=true;setStatus(setup?"正在建立多重防护…":"正在等待通行密钥…");try{const root=setup?await setupVault(passphrase.value):await unlockVault(passphrase.value);post({root:bytesToBase64Url(root),type:"functionhx:spark-vault-unlocked"});setStatus(setup?"私密库已建立，恢复包已下载。":"已安全解锁。","success");setTimeout(()=>window.close(),420)}catch(error){setStatus(error.name==="NotAllowedError"?"通行密钥验证已取消。":error.message||"解锁失败。","error")}finally{submit.disabled=false}});
  document.getElementById("recovery-file").addEventListener("change",async event=>{try{const file=event.target.files?.[0];if(!file)return;const kit=JSON.parse(await file.text());if(kit.type!=="functionhx-spark-vault-recovery"||kit.version!==2)throw new Error("恢复包格式不正确。");const recoveryKey=await crypto.subtle.importKey("raw",base64UrlToBytes(kit.recovery_key),{name:"AES-GCM"},false,["decrypt"]);const ring=kit.keyring;const root=new Uint8Array(await crypto.subtle.decrypt({name:"AES-GCM",iv:base64UrlToBytes(ring.recovery_iv),additionalData:encoder.encode("functionhx:spark-vault-recovery:v2")},recoveryKey,base64UrlToBytes(ring.recovery_wrapped_root)));post({root:bytesToBase64Url(root),type:"functionhx:spark-vault-unlocked"});setStatus("已通过离线恢复包解锁。请尽快重新配置通行密钥。","success");setTimeout(()=>window.close(),500)}catch(error){setStatus(error.message||"无法读取恢复包。","error")}});
})();
</script>
</body>
</html>`;
